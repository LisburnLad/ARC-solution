...
{ // list = 1
  ...
  {
    if (cur.isvec || cur.depth + cost + buffer > MAXDEPTH)
      return false;
    real_f_time.start();
...
Functions3 initFuncs3(const vector<point> &sizes)
{
  ...
  for (int c = 0; c < 10; c++)
    funcs.add("filterCol " + to_string(c), 10, [c](Image_ img)
              { return filterCol(img, c); });
  for (int c = 1; c < 10; c++)
    funcs.add("eraseCol " + to_string(c), 10,
              [c](Image_ img)
              { return eraseCol(img, c); });

  for (int c = 1; c < 10; c++)
    funcs.add("colShape " + to_string(c), 10, [c](Image_ img)
              { return colShape(img, c); }, 0);

  funcs.add("compress", 10, [](Image_ img)
            { return compress(img); });
  ...
  for (int id = 0; id < 3; id++)
    funcs.add("connect " + to_string(id), 10,
              [id](Image_ img)
              { return connect(img, id); });

  for (int id : {0, 1})
    funcs.add("spreadCols " + to_string(id), 10,
              [id](Image_ img)
              { return spreadCols(img, id); });
  ...
  funcs.add(sizes, "repeat 1", 10, [](Image_ a, Image_ b)
            { return repeat(a, b, 1); });
  funcs.add(sizes, "mirror 0", 10, [](Image_ a, Image_ b)
            { return mirror(a, b); });
  funcs.add(sizes, "mirror 1", 10, [](Image_ a, Image_ b)
            { return mirror(a, b, 1); });

  // Split
  funcs.add("cut", 10, [](Image_ img)
            { return cut(img); });
  ...
  for (int id = 0; id < 14; id++)
    funcs.add("pickMaxes " + to_string(id), 10,
              [id](vImage_ v)
              { return pickMaxes(v, id); });
  for (int id = 0; id < 14; id++)
    funcs.add("pickNotMaxes " + to_string(id), 10,
              [id](vImage_ v)
              { return pickNotMaxes(v, id); });
...
void DAG::buildBinary()
{
  ...
  for (int fa : funcs.listed)
  {
    ...
    for (int fb : funcs.listed)
    {
      ...
      if (state[fa].isvec || state[fb].isvec)
        continue;
      State nxt;

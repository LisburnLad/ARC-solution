...
{ // include0 = 1
  if (col.w * col.h == 0 || shape.w * shape.h == 0)
    return badImg;
...
{
  if (shape.w * shape.h == 0 || col.w * col.h == 0)
    return badImg;
...
Image compose(Image_ a, Image_ b, const function<int(int, int)> &f, int overlap_only)
{
  ...
  else if (overlap_only == 2)
  {
    ...
    assert(0);
  if (ret.w > MAXSIDE || ret.h > MAXSIDE || ret.w * ret.h > MAXAREA)
    return badImg;
...
{ // id = 0
  if (id == 0)
  {
    return compose(a, b, [](int a, int b)
                   { return b ? b : a; }, 0); // a then b, inside either
  }
  else if (id == 1)
  {
    return compose(a, b, [](int a, int b)
                   { return b ? b : a; }, 1); // a then b, inside both
  }
  else if (id == 2)
  {
    return compose(a, b, [](int a, int b)
                   { return b ? a : 0; }, 1); // a masked by b
  }
  else if (id == 3)
  {
    return compose(a, b, [](int a, int b)
                   { return b ? b : a; }, 2); // a then b, inside of a
  }
  else if (id == 4)
  {
    return compose(a, b, [](int a, int b)
                   { return b ? 0 : a; }, 2); // a masked by inverse of b, inside of a
  }
  else
    assert(id >= 0 && id < 5);
  return badImg;
...
{
  return compose(Fill(a), a, [](int x, int y)
                 { return y ? 0 : x; }, 0);
}
  ...
  for (int i = 0; i < base.h; i++)
  {
    for (int j = 0; j < base.w; j++)
    {
      if (!done(i, j) && base(i, j))
      {
        ...
        {
          if (r < 0 || r >= base.h || c < 0 || c >= base.w || base(r, c) != acol || done(r, c))
            return;
...
Image count(Image_ img, int id, int outType)
{
  ...
  if (max(sz.x, sz.y) > MAXSIDE || sz.x * sz.y > MAXAREA)
    return badImg;
...
{
  if (line.w * line.h == 0 || area.w * area.h == 0)
    return badImg;
...
{
  return pickMax(v, [id](Image_ img)
                 { return maxCriterion(img, id); });
}
  ...
  for (int i = 0; i < img.h; i++)
  {
    for (int j = 0; j < img.w; j++)
    {
      if (!done(i, j) && !a.safe(i + d.y, j + d.x))
      {
        ...
        function<void(int, int)> dfs = [&](int r, int c)
        {
          if (r < 0 || r >= img.h || c < 0 || c >= img.w || a.safe(r + d.y, c + d.x) || done(r, c))
            return;
          toadd(r, c) = img(r, c) + 1;
          done(r, c) = 1;
          for (int nr : {r - 1, r, r + 1})
...
{
  return pickMaxes(v, [id](Image_ img)
                   { return maxCriterion(img, id); }, 0);
}
vImage pickNotMaxes(vImage_ v, int id)
{
  return pickMaxes(v, [id](Image_ img)
                   { return maxCriterion(img, id); }, 1);
}
...
Image heuristicCut(Image_ img)
{
  ...
  for (int col = 0; col < 10; col++)
  {
    ...
    {
      if (r < 0 || r >= img.h || c < 0 || c >= img.w || img(r, c) != col || done(r, c))
        return;
      done(r, c) = 1;
      for (int nr : {r - 1, r, r + 1})
        for (int nc : {c - 1, c, c + 1})

...
Image makeBorder2(Image_ img, int usemaj = 1)
{
  int bcol = 1;
  if (usemaj)
    bcol = core::majorityCol(img);

  point rsz = img.sz + point{2, 2};
  if (max(rsz.x, rsz.y) > MAXSIDE || rsz.x * rsz.y > MAXAREA)
    return badImg;
...
Image compress3(Image_ img)
{
  ...
  for (int i = 0; i < img.h; i++)
  {
    for (int j = 0; j < img.w; j++)
    {
      if (i && img(i, j) != img(i - 1, j))
        row[i] = 1;
      if (j && img(i, j) != img(i, j - 1))
        col[j] = 1;
    }
  }
  vector<int> rows, cols;
...
{
  if (marked_.sz != need_.sz)
    return badImg;
  if (need_.w * need_.h <= 0)
    return in;
      ...
      for (int j = 0; j + need.w <= ret.w; j++)
      {
        ...
        for (int y = 0; y < need.h; y++)
          for (int x = 0; x < need.w; x++)
            if ((overlapping ? in : ret)(i + y, j + x) != need(y, x))
              ok = 0;
        ...
        if (overlapping == 2)
        {
          for (int y = -1; y <= need.h; y++)
          {
            ...
            {
              if (x >= 0 && y >= 0 && x < need.w && y < need.h)
                continue;

              char nn = need(clamp(y, 0, need.h - 1),
                             clamp(x, 0, need.w - 1));
              if (nn && nn == in.safe(i + y, j + x))
                ok = 0;
            }
          }
        }
    ...
    for (int r = 0; r < rots; r++)
    {
      ...
      for (int i = 0; i + need.h <= ret.h; i++)
      {
        for (int j = 0; j + need.w <= ret.w; j++)
        {
          ...
          for (int y = 0; y < need.h; y++)
            for (int x = 0; x < need.w; x++)
              if (done(i + y, j + x) || ret(i + y, j + x) != need(y, x))
                ok = 0;
          if (ok)
          {
            for (int y = 0; y < need.h; y++)
...
Image smear(Image_ img, int id)
{
  ...
  const vector<pair<int, int>> d[15] = {{R}, {L}, {D}, {U}, {R, L}, {D, U}, {R, L, D, U}, {X}, {Y}, {Z}, {W}, {X, Y}, {Z, W}, {X, Y, Z, W}, {R, L, D, U, X, Y, Z, W}};
  const int w = img.w;
  Image ret = img;

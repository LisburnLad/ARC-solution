...
Image solveTask(const Image &img, const vector<pair<Image, Image>> &train, int taski)
{
  if (taski == 0)
  {
    ...
    // print(filtered);
    return compress(img_2);
    // return embed(img_2, img);
    // return toOrigin(compress(filterCol(img, 1)));
  }
  else if (taski == 1)
  {
    // cutPickMax - compresses
    print(img);
    Image cols = cutPickMax(img, 4);
    print(cols);
    Image shape = cutPickMax(img, 0);
    print(shape);
    Image final = toOrigin(colShape(cols, shape));
    print(final);
    return final;
  }
  else if (taski == 2)
  ...
  {
    print(img);
    // gets shape from output of training image
    Image cutter = filterCol(train[0].second, 1);
    print(cutter);

    // filter the image to only keep colour 4
    Image filtered = filterCol(img, 4);
    print(filtered);

    // add the shape that was cut from training
    Image yellow = compose(cutter, filtered);
    print(yellow);
    ...
    print(positioned);
    Image final = compose(positioned, cutter, 1);
    print(final);
    return final;
  }
  else if (taski == 7)
  {
    ...
    print(red);
    Image move = Move(red, Pos(2, 0));
    print(move);
    Image final = compose(blue, move, 0);
    print(final);
    return final;
  }
  else if (taski == 8)
